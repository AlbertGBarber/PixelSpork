...
ColorWipeSLSeg::ColorWipeSLSeg(SegmentSetPS &SegSet, palettePS &Palette, patternPS &Pattern, uint16_t WipeLength,
                               uint8_t Style, bool Simult, bool Alternate, bool WipeDirect, bool SegMode,
                               uint16_t Rate)
    : palette(&Palette), pattern(&Pattern), wipeLength(WipeLength), style(Style), simult(Simult),
      alternate(Alternate), wipeDirect(WipeDirect), segMode(SegMode)  //
{
    init(SegSet, Rate);
}

//Constructor using palette alone
ColorWipeSLSeg::ColorWipeSLSeg(SegmentSetPS &SegSet, palettePS &Palette, uint16_t WipeLength, uint8_t Style,
                               bool Simult, bool Alternate, bool WipeDirect, bool SegMode, uint16_t Rate)
    : palette(&Palette), wipeLength(WipeLength), style(Style), simult(Simult), alternate(Alternate),
      wipeDirect(WipeDirect), segMode(SegMode)  //
{
    setPaletteAsPattern();
    init(SegSet, Rate);
}
...
ColorWipeSLSeg::ColorWipeSLSeg(SegmentSetPS &SegSet, CRGB WipeColor, uint16_t WipeLength, uint8_t Style,
                               bool Simult, bool Alternate, bool WipeDirect, bool SegMode, uint16_t Rate)
    : wipeLength(WipeLength), style(Style), simult(Simult), alternate(Alternate), wipeDirect(WipeDirect),
      segMode(SegMode)  //
{
...
void ColorWipeSLSeg::init(SegmentSetPS &SegSet, uint16_t Rate) {
