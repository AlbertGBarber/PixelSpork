#include "PoliceStrobePS.h"

//constructor for a traditional two color strobe
PoliceStrobePS::PoliceStrobePS(SegmentSet &SegmentSet, CRGB ColorOne, CRGB ColorTwo, CRGB BgColor, uint8_t NumPulses, uint16_t PauseTime, uint8_t PulseMode, uint16_t Rate):
...
PoliceStrobePS::PoliceStrobePS(SegmentSet &SegmentSet, palletPS *Pallet, CRGB BgColor, uint8_t NumPulses, uint16_t PauseTime, uint8_t PulseMode, uint16_t Rate):
    segmentSet(SegmentSet), pallet(Pallet), numPulses(NumPulses), pauseTime(PauseTime), pulseMode(PulseMode)
    {    
        init(BgColor, Rate); 
	}

PoliceStrobePS::~PoliceStrobePS(){
    delete[] palletTemp.palletArr;
}

//restarts the effect
void PoliceStrobePS::reset(){
    colorNum = 0;
    pulseCount = 1;
    pause = false;
    prevGuess = 0;
...
void PoliceStrobePS::init(CRGB BgColor, uint16_t Rate){
...
void PoliceStrobePS::update(){
    currentTime = millis();

    if( ( currentTime - prevTime ) >= *rate ) {
        prevTime = currentTime;
        //get the current strip length and
...
void PoliceStrobePS::startPause(){
    if(fillBGOnPause){
        segDrawUtils::fillSegSetColor(segmentSet, *bgColor, bgColorMode);
    }
