...
void ParticlesSLPS::init(CRGB BgColor){
...
void ParticlesSLPS::setParticleSet(particleSetPS *newParticleSet){
    ...
    if( ( currentTime - prevTime ) >= updateRate ) {
        ...
        for (uint8_t i = 0; i < numParticles; i++) {
            ...
            if ( trailType < 4 && (!fillBG && !blend) ) {
                ...
                //segDrawUtils::getPixelColor(segmentSet, &pixelInfo, *bgColor, bgColorMode, trailLineNum);
            ...
            for(uint8_t j = 0; j < numSegs; j++){
                ...
                for (uint16_t k = 0; k < size; k++) {
                    ...
                    colorFinal = segDrawUtils::getPixelColor(segmentSet, pixelNum, partColor, colorMode, j, trailLineNum);
...
void ParticlesSLPS::moveParticle(particlePS *particlePtr) {
...
void ParticlesSLPS::setTrailColor(uint16_t trailLineNum, uint8_t segNum, uint8_t trailPixelNum){
    ...
    colorFinal = segDrawUtils::getPixelColor(segmentSet, pixelNum, partColor, colorMode, segNum, trailLineNum);
...
uint16_t ParticlesSLPS::getTrailLedLoc(bool trailDirect, uint8_t trailPixelNum, uint16_t maxPosition) { 
    //get the multiplier for the direction (1 or -1)
