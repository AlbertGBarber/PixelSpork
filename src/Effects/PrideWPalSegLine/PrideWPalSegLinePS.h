#ifndef PrideWPalSegLinePS_h
#define PrideWPalSegLinePS_h
...
Passing in a segmentSet with one segment with all your leds in order will produce the oringal effect
(set briDirect = true and randomBriInc = false)

The effect is different from PrideWPalSegLine because the colors move along the segment lines, rather then along the segments
...
Example calls:
    PrideWPalSegLinePS(mainSegments, true, false, 80);
    ...
    PrideWPalSegLinePS(ringSegments, 4, false, true, 80);
    ...
    PrideWPalSegLinePS(ringSegments, &pallet1, true, true, 80);
    Will do an effect using colors from pallet1
    The brightness waves will move with the colors
    The brightness wave values will be randomized, see init() for the ranges
    The effect updates at 80ms

    PrideWPalSegLinePS(ringSegments, &pallet1, true, 20, 120, 250, 350, 20, 40, 3, 7, 60);
    Will do an effect using colors from pallet1
    The brightness waves will move with the colors
...
*/
class PrideWPalSegLinePS : public EffectBasePS {
    public:
        //constructor for rainbow mode
        PrideWPalSegLinePS(SegmentSet &SegmentSet, bool BriDirect, bool RandomBriInc, uint16_t Rate);

        //constructor for pallet input
        PrideWPalSegLinePS(SegmentSet &SegmentSet, palletPS *Pallet, bool BriDirect, bool RandomBriInc, uint16_t Rate);  

        //constructor for making a random pallet
        PrideWPalSegLinePS(SegmentSet &SegmentSet, uint8_t numColors, bool BriDirect, bool RandomBriInc, uint16_t Rate);

        //constructor with inputs for all main variables
        PrideWPalSegLinePS(SegmentSet &SegmentSet, palletPS *Pallet, bool BriDirect, uint8_t GradLength, 
