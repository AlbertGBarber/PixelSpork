#include "RollingWavesSLPS.h"

//constructor with pattern
RollingWavesSLPS::RollingWavesSLPS(SegmentSet &SegmentSet, patternPS *Pattern, palettePS *Palette, CRGB BGColor, uint8_t GradLength, uint8_t TrailMode, uint8_t Spacing, uint16_t Rate):
...
RollingWavesSLPS::RollingWavesSLPS(SegmentSet &SegmentSet, palettePS *Palette, CRGB BGColor, uint8_t GradLength, uint8_t TrailMode, uint8_t Spacing, uint16_t Rate):
...
RollingWavesSLPS::RollingWavesSLPS(SegmentSet &SegmentSet, uint8_t NumColors, CRGB BGColor, uint8_t GradLength, uint8_t TrailMode, uint8_t Spacing, uint16_t Rate):
...
RollingWavesSLPS::~RollingWavesSLPS(){
    delete[] paletteTemp.paletteArr;
    delete[] patternTemp.patternArr;
}

//inits core variables for the effect
void RollingWavesSLPS::init(CRGB BgColor, uint16_t Rate){
...
void RollingWavesSLPS::setGradLength(uint8_t newGradLength){
...
void RollingWavesSLPS::setSpacing(uint8_t newSpacing){
    spacing = newSpacing;
    setTotalEffectLength();
}
...
//we need to change the totalCycleLength to match
void RollingWavesSLPS::setPattern(patternPS *newPattern){
    pattern = newPattern;
    setTotalEffectLength();
}
...
//{0, 1, 2, 3, 4}
void RollingWavesSLPS::setPaletteAsPattern(){
...
void RollingWavesSLPS::setTotalEffectLength(){
...
void RollingWavesSLPS::setTrailMode(uint8_t newTrailMode){
    ...
    if( ( currentTime - prevTime ) >= *rate ) {
        ...
        for (uint16_t i = 0; i < numLines; i++) {
            ...
            for(uint16_t j = 0; j < numSegs; j++){
                ...
                if(setBg){
                    colorOut = segDrawUtils::getPixelColor(segmentSet, pixelNum, *bgColor, bgColorMode, j, lineNum);
                } else{
                    colorOut = segDrawUtils::getPixelColor(segmentSet, pixelNum, currentColor, colorMode, j, lineNum);
                ...
                segDrawUtils::setPixelColor(segmentSet, pixelNum, colorOut, 0, j, lineNum);
            }
        }
...
CRGB RollingWavesSLPS::desaturate(CRGB color, uint8_t step, uint8_t totalSteps) {
...
void RollingWavesSLPS::setNextColors(uint16_t segPixelNum){
