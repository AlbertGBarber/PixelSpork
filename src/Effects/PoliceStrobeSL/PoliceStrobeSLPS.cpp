#include "PoliceStrobeSLPS.h"

//constructor for a traditional two color strobe
PoliceStrobeSLPS::PoliceStrobeSLPS(SegmentSet &SegmentSet, CRGB ColorOne, CRGB ColorTwo, CRGB BgColor, uint8_t NumPulses, uint16_t PauseTime, uint8_t PulseMode, uint16_t Rate):
...
void PoliceStrobeSLPS::init(CRGB BgColor, uint16_t Rate){
...
void PoliceStrobeSLPS::startPause(){
    if(fillBGOnPause){
        segDrawUtils::fillSegSetColor(segmentSet, *bgColor, bgColorMode);
    }
