...
Example calls: 
    NoiseSL(mainSegments, 4, 20, 20, 60, 10, 80);
    ...
    NoiseSL(mainSegments, &palette1, 40, 5, 95, 20, 80);
...
class NoiseSL : public EffectBasePS {
    public:
        ...
        NoiseSL(SegmentSet &SegmentSet, uint8_t numColors, uint16_t BlendSteps, uint16_t ScaleBase, uint16_t ScaleRange, uint16_t Speed, uint16_t Rate);

        //Constructor using palette
        NoiseSL(SegmentSet &SegmentSet, palettePS *Palette, uint16_t BlendSteps, uint16_t ScaleBase, uint16_t ScaleRange, uint16_t Speed, uint16_t Rate); 
