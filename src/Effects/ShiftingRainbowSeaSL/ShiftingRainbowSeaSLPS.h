#ifndef ShiftingRainbowSeaSLPS_h
#define ShiftingRainbowSeaSLPS_h
...
Example calls: 
    ShiftingRainbowSeaSLPS(mainSegments, 0, 3, 30);
    ...
    ShiftingRainbowSeaSLPS(mainSegments, 10, 3, 30);
...
Notes:
*/
class ShiftingRainbowSeaSLPS : public EffectBasePS {
    public:
        //Constructor for the effect. Note that by passing a gradLength of 0, you will set the effect to sMode 0
        //In sMode 0 the gradLength isn't used, but to make sure that if you set it back to sMode 0,
        //the gradlength will be set to random value between 20 - 40
        ShiftingRainbowSeaSLPS(SegmentSet &SegmentSet, uint8_t GradLength, uint8_t Grouping, uint16_t Rate);
        
        //destructor
        ~ShiftingRainbowSeaSLPS();
