...
Example calls: 
    ...
    BreathPS(mainSegments, CRGB::Red, CRGB::Blue, 5, 50);
...
Other Settings:
    ...
    breathCheckLim (default 250) -- The brightness threshold for changing colors,
...
class BreathPS : public EffectBasePS {
    public:
        ...
        BreathPS(SegmentSet &SegmentSet, CRGB color, CRGB BgColor, uint8_t BreathFreq, uint16_t Rate); 
        ...
        uint8_t
            ...
            *breathFreq, //pointer to the breath frequency (dby default it's bound to the the Rate)
            breathCheckLim = 250, //The limit above which a new color is choosen (see comments in update() code)
            rainbowSat = 255,
            rainbowVal = 255,
            rainbowRate = 20,
