...
BreathPS::BreathPS(SegmentSet &SegmentSet, CRGB color, CRGB BgColor, uint8_t BreathFreq, uint16_t Rate):
...
void BreathPS::update(){
    ...
    if( (currentTime - prevTime) >= *rate ) {
        ...
        breath = map8(breath, 60, 255); //map from 0, 255 to 60, 255;
        ...
        //From experimentation 250 seems to work well. The highest possible peak is 255.
        //We lock the color because the wave may stay above 250 for multiple cycles, but we only want to set the color once.
        ...
        if(breath >= breathCheckLim && !lockColor){
            getNextColor();
            lockColor = true;
        } else if(lockColor && breath < breathCheckLim){
            lockColor = false;
        }
