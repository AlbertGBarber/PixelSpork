#include "PrideWPalSLPS.h"

//constructor for rainbow mode
PrideWPalSLPS::PrideWPalSLPS(SegmentSet &SegmentSet, bool BriDirect, bool RandomBriInc, uint16_t Rate):
...
void PrideWPalSLPS::init(bool RandomBriInc, uint16_t Rate){
...
void PrideWPalSLPS::randomizeBriInc(uint8_t briThetaMinMin, uint8_t briThetaMinMax, 
...
void PrideWPalSLPS::randomizeBriFreq( uint16_t briFreqMin, uint16_t briFreqMax ){
    ...
    if( ( deltams ) >= *rate ) {
        ...
        for (uint16_t i = 0; i < numLines; i++) {
            ...
            for (uint8_t j = 0; j < numSegs; j++) {
                ...
                if(pixelNum != dLed){ //avoid outputing to dummy leds
