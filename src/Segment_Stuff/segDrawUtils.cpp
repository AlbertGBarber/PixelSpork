...
uint16_t segDrawUtils::getSegmentPixel(SegmentSet &SegSet, uint16_t segPixelNum){
...
void segDrawUtils::getSegLocationFromPixel(SegmentSet &SegSet, uint16_t segPixelNum, uint16_t locData1[2] ){
   lengthSoFar = 0;
   ...
   //so we set the led location to D_LED, which will be ignored when trying to write out any colors
   locData1[0] = 0;
   locData1[1] = D_LED; 
...
uint16_t segDrawUtils::getSegmentPixel(SegmentSet &SegSet, uint16_t segNum, uint16_t segPixelNum) {
...
void segDrawUtils::fillSegSetColor(SegmentSet &SegSet, const CRGB &color, uint8_t colorMode){
    for(uint16_t i = 0; i < SegSet.numSegs; i++){
...
void segDrawUtils::fillSegColor(SegmentSet &SegSet, uint16_t segNum, const CRGB &color, uint8_t colorMode) {
    numSec = SegSet.getTotalNumSec(segNum);
    lengthSoFar = 0;
...
//Where secNum is the section you're coloring in
        ...
        //for(uint8_t i = 0; i < secNum; i++){
            //lengthSoFar += SegSet.getSecLength(segNum, i);
        //}
void segDrawUtils::fillSegSecColor(SegmentSet &SegSet, uint16_t segNum, uint16_t secNum, uint16_t pixelCount, const CRGB &color, uint8_t colorMode ){
...
void segDrawUtils::fillSegLengthColor(SegmentSet &SegSet, uint16_t segNum, uint16_t startSegPixel, uint16_t endPixel, const CRGB &color, uint8_t colorMode){
...
//(starting from 0)
void segDrawUtils::fillSegSetLengthColor(SegmentSet &SegSet, uint16_t startSegPixel, uint16_t endPixel, const CRGB &color, uint8_t colorMode ){
...
void segDrawUtils::drawSegLine(SegmentSet &SegSet, uint16_t lineNum, const CRGB &color, uint8_t colorMode) {
...
void segDrawUtils::drawSegLineSection(SegmentSet &SegSet, uint16_t startSeg, uint16_t endSeg, uint16_t lineNum, const CRGB &color, uint8_t colorMode) {
...
uint16_t segDrawUtils::getPixelNumFromLineNum(SegmentSet &SegSet, uint16_t numLines, uint16_t segNum, uint16_t lineNum) { 
...
uint16_t segDrawUtils::getLineNumFromPixelNum(SegmentSet &SegSet, uint16_t segPixelNum){
...
uint16_t segDrawUtils::getLineNumFromPixelNum(SegmentSet &SegSet, uint16_t segPixelNum, uint16_t segNum){
...
void segDrawUtils::setPixelColor(SegmentSet &SegSet, uint16_t segPixelNum, const CRGB &color, uint8_t colorMode){
    getSegLocationFromPixel(SegSet, segPixelNum, locData1);
...
void segDrawUtils::setPixelColor(SegmentSet &SegSet, uint16_t segPixelNum, const CRGB &color, uint8_t colorMode, uint16_t segNum){
...
void segDrawUtils::setPixelColor(SegmentSet &SegSet, uint16_t pixelNum, const CRGB &color, uint8_t colorMode, uint16_t segNum, uint16_t lineNum){
    if( pixelNum == D_LED ){
...
void segDrawUtils::handleBri(SegmentSet &SegSet, uint16_t pixelNum){
...
void segDrawUtils::getPixelColor(SegmentSet &SegSet, uint16_t segPixelNum, pixelInfoPS *pixelInfo, const CRGB &color, uint8_t colorMode){
    getSegLocationFromPixel(SegSet, segPixelNum, locData1);
...
void segDrawUtils::show(SegmentSet &SegSet, bool showNow){
...
void segDrawUtils::setGradOffset(SegmentSet &SegSet, uint16_t offsetMax){
    if(SegSet.runOffset){
        currentTime = millis();
...
void segDrawUtils::fadeSegSetToBlackBy(SegmentSet &SegSet, uint8_t val){
    for(uint16_t i = 0; i < SegSet.numSegs; i++){
...
//uses FastLED's fadeToBlackBy function
void segDrawUtils::fadeSegToBlackBy(SegmentSet &SegSet, uint16_t segNum, uint8_t val) {
    numSec = SegSet.getTotalNumSec(segNum);
...
//uses FastLED's fadeToBlackBy function
void segDrawUtils::fadeSegSecToBlackBy(SegmentSet &SegSet, uint16_t segNum, uint16_t secNum, uint8_t val){
    secLength = SegSet.getSecLength(segNum, secNum);
