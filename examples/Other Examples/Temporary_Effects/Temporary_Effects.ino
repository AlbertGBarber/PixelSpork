...
/*
In this example, we'll create "temporary" effect, which only keep their variables in memory while active.
This allows effects to "share" memory, so you can use a larger number of effects overall.

Think of your micro-controller's memory as a stage. As I add more acts (effects), the stage gets more and more 
crowded -- there's only so much space!
Meanwhile, lets say I only have one act active at one time. There's no point in having everyone else up there
all the time. Instead it would be better to have people wait back stage (where there's more room) and 
only come on as needed, making them "temporary". This is the idea behind temporary effects. 

While this sounds great, in practice it is more complicated, making it an advanced technique, 
which requires some knowledge of memory management, and can easily lead to crashes if not coded carefully. 
YOU SHOULD AVOID USING TEMPORARY EFFECTS UNLESS YOU ABSOLUTELY NEED THEM.
See the "Temporary Effects" wiki page: https://github.com/AlbertGBarber/PixelSpork/wiki/Temporary-Effects
for a full explanation.

This example is more or less the same as the "Cycling Using Effect Sets" example, but 
we'll use temporary effects instead of pre-allocated ones. 
...
The Palette Blender will be created as normal, while the Twinkle and Streamer effects
will be created during runtime as "temporary" effects.

We'll also create an EffectSetPS to manage the effects. Note that Effect Sets are critical
for temporary effects, as they allow you to easily free their memory. 
...
//Note that we also set a "destruct limit" of 1. When we cycle between our two effects,
//we need to "destruct" or "free" the current effect to release its memory (not doing this leads to a memory leak).
//However, our palette blender is not allocated dynamically, so we NEVER want to destruct it.
//Effect Sets allow us to destruct all the effects in the effect array automatically, while 
//also setting a starting point in the array for the destruction, the "destruct limit".
//Only effects after the limit in the array will be destucted. So in our case we set it at 1,
//placing the palette blender in the first array index, and the effects in the second,
//so only the effect will be destructed.

//======================================================================================
//With the Effect Set created we can create our effects.
...
//Note that because we will create the effects during runtime, we only need to 
//create pointers to them for now. We will tie the pointers to the effects at runtime. 
TwinkleSL *myTwinkle;
...
void setup() {
    ...
    /*
    Up to this point, our effect set is empty, we need to fill it with our palette blender.
    ...
    However, because the palette blender is already defined, we want to add it now.
    ...
    Remember, its location in the array is important due to the destruct limit we set when defining the effect set.
    */ 
...
Overall the cycle looks like:   
    ...
    3. Use the switch() to go to the current effect
        3a. If we haven't setup the effect, do so by creating it, and adding it to our effect set.
    ...
    5. Check if the effect is has finished its run time using the Effect Sets "done" flag.
       5a. If so, de-allocate the current effect, reset() the effect set,
           and tell the code we need to setup the next effect.
...
void loop() {
    ...
    switch(effectNum) {
        ...
        default: {  //effect 0, myStreamer
            ...
            if(!effectSetup) {
                //create the Streamer effect dynamically using "new"
        ...
        case 1: {  //effect 1, myTwinkle
            //If we are just starting the effect, we need to add it to the effect set, and set a run time
            if(!effectSetup) {
                //create the Twinkle effect dynamically using "new"
    ...
    de-allocate the memory of the current effect, set the effectSetup flag, 
    so we know to setup the next effect in the switch statement above,
